...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { templateId, title, category } = await request.json();

    const admin = getAdminClient();
    const { data, error } = await admin
      .from("platform_templates")
      .update({ title: String(title), category: String(category) })
      .eq("id", String(templateId))
      .select("*")
      .single();

    if (error || !data) {
      throw new Error("Nao foi possivel atualizar moldura.");
    }
